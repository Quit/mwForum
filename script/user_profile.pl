...
if (!$submitted || @{$m->{formErrors}}) {
	...
	push @userLinks, { url => $m->url('user_avatar', uid => $optUserId), 
		...
		if $cfg->{avatars};
	push @userLinks, { url => $m->url('user_badges', uid => $optUserId), 
		txt => 'uopBadges', ico => 'tag' } 
		if @{$cfg->{badges}} && ($selfBadge || $user->{admin});
	...
	print
		...
		"<input type='text' class='fcs qwi' name='realName' maxlength='100'",
		" autofocus='autofocus' value='$realNameEsc'/></label>\n",
