...
if ($submitted) {
	...
	# Process upload form
		...
		if (!@{$m->{formErrors}}) {
			...
			mkdir $keyFsPath or $m->error("Key directory creation failed. ($!)") if !-d $keyFsPath;
...
$m->printHints(["You can have your emails encrypted for you by uploading your" .
...
print
	...
	"<textarea class='fcs' name='key' rows='4' autofocus='autofocus' required='required'>",
	"</textarea></label>\n",
...
if (-s $keyFile) {
	my ($in, $out, $err);
	my $cmd = [
		$gpgPath, "--batch", "--no-auto-check-trustdb", "--charset" => "utf-8", "-v",
		"--no-default-keyring", "--keyring" => $keyFile,
		$cfg->{gpgOptions} ? @{$cfg->{gpgOptions}} : (),
		"--list-keys",
	];
...
if (!$result) {
	my ($in, $out, $err);
	my $cmd = [
		$gpgPath, "--batch", "--no-auto-check-trustdb", "--charset" => "utf-8", "--armor",
		$cfg->{gpgOptions} ? @{$cfg->{gpgOptions}} : (),
		"--export" => $cfg->{gpgSignKeyId}
	];
