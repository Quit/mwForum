...
if ($submitted) {
	...
	if (!@{$m->{formErrors}}) {
		# Get boards and delete subscriptions
		...
		for my $board (@$boards) {
			...
			$m->dbDo("
				INSERT INTO boardSubscriptions (userId, boardId, instant) VALUES (?, ?, ?)", 
				$optUserId, $boardId, $instant)
				if $subscribe;
...
if (!$submitted || @{$m->{formErrors}}) {
	...
	for my $board (@$boards) {
		my $boardId = $board->{id};
		my $subsDsbl = !$optUser->{email} || $optUser->{dontEmail};
		my $instantDsbl = !$cfg->{subsInstant} || $subsDsbl ? "disabled='disabled'" : "";
		my $digestDsbl = !$cfg->{subsDigest} || $subsDsbl ? "disabled='disabled'" : "";
		my $instantChk = $board->{subscribed} && $board->{instant} ? "checked='checked'" : "";
		my $digestChk = $board->{subscribed} && !$board->{instant} ? "checked='checked'" : "";
		my $unsubChk = $instantChk || $digestChk ? "" : "checked='checked'";
		my $hidden = $board->{hidden} ? "checked='checked'" : "";
		...
		print
			...
			" $instantChk $instantDsbl/> $lng->{ubdBrdStInst}</label>\n",
			"<label><input type='radio' name='subscribe_$boardId' value='1'",
			" $digestChk $digestDsbl/> $lng->{ubdBrdStDig}</label>\n",
